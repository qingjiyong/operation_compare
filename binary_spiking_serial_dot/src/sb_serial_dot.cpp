#include "sb_serial_dot.h"

void load_and_unpack(
    const int rounds,
    Pack_Data *data,
    Pack_Weight *weight,
    hls::stream<hls::vector<Spike_t, ENTRIES>> &spike_stream,
    ...
    for(int r = 0; r < rounds; r++) {
        ...
        hls::vector<Smem, ENTRIES> spike_smem_vec;
        unpack_weight_loop:
        ...
        // unpack data and convert to spikes serially
        Spike_t spikes[ENTRIES][TRANS_LEN];
        #pragma HLS ARRAY_PARTITION variable=spikes complete dim=2

        unpack_data_loop:
        for (int i = 0; i < ENTRIES; i++) {
            #pragma HLS UNROLL
            ap_int<DATA_BITS> temp_spike_val = packed_data.range((i+1)*DATA_BITS-1, i*DATA_BITS);
            
            ap_uint<DATA_BITS> abs_val;
            ap_int<SPIKE_BITS> sign;

            if (temp_spike_val < 0) {
                abs_val = -temp_spike_val;
                sign = -1;
            } else {
                abs_val = temp_spike_val;
                sign = 1;
        ...
        // Stream spikes serially over TRANS_LEN cycles
        stream_spike_loop:
        for(int j = 0; j < TRANS_LEN; j++) {
...
void compute(
    ...
    for(int r = 0; r < rounds; r++) {
        ...
        for (int j = 0; j < TRANS_LEN; j++) {
            ...
            Result_t partial_sum = 0;

            dot_product_loop:
            for (int i = 0; i < ENTRIES; i++) {
                #pragma HLS UNROLL
                if(spiking_data[i] != 0) {
                    partial_sum += spiking_data[i] * weight_vec[i];
                    #pragma HLS BIND_OP variable=partial_sum op=mul impl=fabric
                }
            }
            // Accumulate with bit-shift for positional weight
            acc_result += (partial_sum * (1 << j));
        }
...
void sb_serial_dot(
    ...
    hls::stream<Spiking_Block> spike_stream;
    #pragma HLS STREAM variable=spike_stream depth=4
